			...
			<div v-else class="card">
				<div class="card-header">
					<div class="d-flex justify-content-between">
						<span><strong>Baseball</strong></span>
						<span><strong>{{game.game_time.time}}</strong></span>
					</div>
				</div>
				<div class="card-body">
					<div class="mb-3 d-flex justify-content-between align-items-center">
						<span><img style="height: 40px; width: auto; margin-right: 10px;" 
							   ...
							   <a :href="'/baseball/' + game.the_year.year + '/' + game.away_team.school_name">{{game.away_team.school_name}}</a>
						</span>
					...
					<div class="d-flex justify-content-between align-items-center">
						<span><img style="height: 40px; width: auto; margin-right: 10px;" 
							  ...
							  <a :href="'/baseball/' + game.the_year.year + '/' + game.home_team.school_name">{{game.home_team.school_name}}</a>
						</span>
						<span v-if="game.inning > 0 && game.inning < 99">
							{{homeTeamTotal}}
						</span>
...
<script>
	...
	export default {
		...
		methods: {
			fetchEvent() {
				axios.get('/api/baseball/' + this.id)
				.then(res =>  {
	                    this.game = res.data,
